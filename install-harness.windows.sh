...
detect_cli_platform() {
  ...
  case "$os:$arch" in
    ...
    MINGW64_*:x86_64|MSYS_*:x86_64|CYGWIN_*:x86_64) printf 'windows-x64' ;;
    MINGW64_*:arm64|MSYS_*:arm64|CYGWIN_*:arm64) printf 'windows-arm64' ;;
...
install_harness_cli_binary() {
  ...
  if [[ "$platform" == windows-* ]]; then
    binary_name="harness-cli-$platform.exe"
    target="$TARGET_DIR/scripts/bin/harness-cli.exe"
  else
